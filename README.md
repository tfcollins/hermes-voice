...
A voice-first cinematic HUD for Hermes Agent on Linux. It uses browser audio capture with voice activity detection, local `faster-whisper` transcription, Hermes' authenticated Sessions streaming API, and an original British neural TTS voice.
...
```bash
uv sync --extra test
HERMES_API_KEY=... uv run uvicorn hermes_voice.app:app --host 127.0.0.1 --port 8765
```
